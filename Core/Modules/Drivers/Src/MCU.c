/// @file MCU.c
///
/// @brief STM32G0 MCU peripheral driver — internal ADC, RTC, power monitoring, and CRC.
///
...
/// MCUProcess(). Optional sub-systems compile away entirely when their
/// corresponding FEATURE_ flag is 0.
///
...
#if FEATURE_HARDWARE_CRC
#include "crc.h"
#endif // FEATURE_HARDWARE_CRC

extern osSemaphoreId_t CRCSemHandle;
...
/// @brief ADC DMA buffer indices for internal MCU channels.
...
#if !FEATURE_HARDWARE_CRC
    uint32_t         crcValue;      ///< Running CRC accumulator for software path
#endif // !FEATURE_HARDWARE_CRC
#if FEATURE_RTC
...
// =============================================================================
// VCC — used internally by temperature and battery calculations even when the
// public API is disabled, so the definition gate is broader than FEATURE_VCC_MONITOR.
// =============================================================================
...
#if FEATURE_MCU_TEMP_MONITOR
/// @brief Compute the internal junction temperature from the on-chip ADC channel.
...
/// @brief Compute the battery charge level as a permille fraction (0–1000).
...
void MCUProcess( void ) {
    ...
    uint32_t nonFaultMask = BIT( FlagMCUStatusReady ) | BIT( FlagMCUStatusCrcBusy );
#if FEATURE_RTC
    nonFaultMask |= BIT( FlagMCUStatusTimePending );
#endif // FEATURE_RTC
...
// =============================================================================
// CRC-32 — hardware (FEATURE_HARDWARE_CRC) or software fallback.
// Both paths use polynomial 0x04C11DB7, no reflection, initial value 0xFFFFFFFF,
// no final XOR, matching the STM32 CRC peripheral default configuration exactly.
// =============================================================================

#if FEATURE_HARDWARE_CRC

/// @brief Acquire the CRC engine and reset the accumulator. Blocks until the engine is free.
void CRCInit( MCURef mcu ) {
    if ( mcu == NULL ) return;
    osSemaphoreAcquire( CRCSemHandle, osWaitForever );
    osEventFlagsSet( mcu->statusHandle, BIT( FlagMCUStatusCrcBusy ) );
...
void CRCUpdate( MCURef mcu, const uint8_t* data, size_t length ) {
    UNUSED( mcu );
    HAL_CRC_Accumulate( &hcrc, (uint32_t*)data, (uint32_t)length );
}

/// @brief Return the final CRC-32 value and release the CRC engine.
uint32_t CRCResult( MCURef mcu ) {
    if ( mcu == NULL ) return 0;
    uint32_t value = hcrc.Instance->DR;
    osEventFlagsClear( mcu->statusHandle, BIT( FlagMCUStatusCrcBusy ) );
    osSemaphoreRelease( CRCSemHandle );
    return value;
}

#else // software fallback

// CRC-32/MPEG-2: polynomial 0x04C11DB7, MSB-first, no reflection, no final XOR.
// Matches the STM32 hardware CRC peripheral default configuration exactly.
// Table source: https://www.mrob.com/pub/comp/crc-all.html
static const uint32_t sCrcTable[ 256 ] = {
...
void CRCUpdate( MCURef mcu, const uint8_t* data, size_t length ) {
    ...
    for ( size_t i = 0; i < length; i++ ) {
        mcu->crcValue = ( mcu->crcValue << 8 ) ^ sCrcTable[ ( mcu->crcValue >> 24 ) ^ data[ i ] ];
    }
}
...
/// @brief Convenience wrapper: compute CRC-32 over a single contiguous buffer.
uint32_t CRCCompute( MCURef mcu, const uint8_t* data, size_t length ) {
    CRCInit( mcu );
    CRCUpdate( mcu, data, length );
    return CRCResult( mcu );
}

/// @brief Convenience wrapper: compute CRC-32 and compare against an expected value.
bool CRCVerify( MCURef mcu, const uint8_t* data, size_t length, uint32_t expected ) {
    return CRCCompute( mcu, data, length ) == expected;
}
