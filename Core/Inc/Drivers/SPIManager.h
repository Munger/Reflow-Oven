...
///
/// Wraps HAL SPI interrupt-driven and blocking transfers behind a per-instance
...
///
/// Each SPI bus is identified by SPIID and opened with SPIOpen(). All transfer
/// functions take the returned SPIRef as their first argument — no HAL handle
...
/// @brief Logical identifiers for SPI bus instances managed by this driver.
typedef enum {
    SPIBus1 = 0,    ///< SPI1 — flash and thermocouple bus
    SPIBusCount
} SPIID;

/// @brief Opaque handle to an SPI bus instance.
typedef struct SPIInstance* SPIRef;

/// @brief Status and diagnostic flag bit positions for an SPI bus instance.
